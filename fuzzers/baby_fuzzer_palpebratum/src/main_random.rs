...
use libafl::{corpus::{InMemoryOnDiskCorpus, OnDiskCorpus}, events::SimpleEventManager, executors::{TcpExecutor, WithObservers}, feedbacks::{RandomFeedback, NotFeedback}, fuzzer::{Fuzzer, StdFuzzer}, generators::SeedFileGenerator, mutators::scheduled::{havoc_mutations, StdScheduledMutator}, schedulers::QueueScheduler, stages::mutational::StdMutationalStage, state::StdState};
use libafl::feedbacks::{NeverInterestingFeedback,CrashFeedback};
...
pub fn main() {
    ...
    //let observer = unsafe { ConstMapObserver::<u8, NUMBER_OF_NODES>::from_mut_ptr("signals", SIGNALS_PTR) };

    // Feedback to rate the interestingness of an input
    let mut feedback = RandomFeedback::new();
